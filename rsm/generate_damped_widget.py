...
# Position plot
position_plot.line(
    "x", "y", source=source_position, line_width=2, color="#2c3e50", legend_label="Position x(t)"
)

# Phase plot
phase_plot.line(
    "x", "y", source=source_phase, line_width=2, color="#3498db", legend_label="Trajectory"
)
phase_plot.scatter(
    "x", "y", source=source_start, size=10, color="#27ae60", legend_label="Start", marker="circle"
)
phase_plot.scatter(
    "x", "y", source=source_end, size=10, color="#e74c3c", legend_label="End", marker="square"
)
...
# Sliders
omega0_slider = Slider(
    start=0.5, end=5, value=omega0_init, step=0.1, title="Natural Frequency ω₀ (rad/s)"
)
gamma_slider = Slider(
    start=0, end=5, value=gamma_init, step=0.1, title="Damping Coefficient γ (s⁻¹)"
)
...
with open("/Users/leo.torres/aris/press/seed_papers/damped_oscillator_widget.html", "w") as f:
    ...
    f.write(
        "<style>.bk-root > .bk-Column > .bk-Row { display: flex !important; justify-content: center !important; }</style>\n"
    )
    # Include the div
    f.write(div)
    f.write("\n")
