...
def laplacian_degrees(graph):
    ...
    degs = sparse.diags(
        np.asarray(adj.sum(axis=1)).ravel(), offsets=0, shape=adj.shape, format="csr"
    )
...
def eigenmaps(graph, dim, method="glee", return_vals=False):
    ...
    eig_fun = {
        "glee": {
            True: np.linalg.eigh,
            False: lambda m: sparse.linalg.eigsh(m, k=dim, which="LM", return_eigenvectors=True),
        },
        "eigen": {
            True: np.linalg.eig,
    ...
    matrix_fun = {
        "glee": {True: lambda: lapl.toarray(), False: lambda: lapl},
        "eigen": {
            True: lambda: invdegs.toarray().dot(lapl.toarray()),
            False: lambda: invdegs.dot(lapl),
        },
    }
    post_process = {
        "glee": lambda vals, vecs: (vals, vecs.dot(np.diag(np.sqrt(vals)))),
        "eigen": lambda vals, vecs: (
            vals[1:],
            vecs[:, 1:].dot(
                np.diag([1 / np.sqrt(v.dot(degs.toarray()).dot(v)) for v in vecs[:, 1:].T])
            ),
        ),
    }
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input", type=str, required=True, help="edge list file path")
    parser.add_argument("--output", type=str, required=True, help="embedding output file path")
    parser.add_argument(
        "--dim", default=128, type=int, help="dimension of embedding (default 128)"
    )
    parser.add_argument("--method", default="glee", type=str, help='"glee" or "eigen"')
    parser.add_argument(
        "--nodetype", default="int", type=str, help="type of node labels in input file"
    )
    args = parser.parse_args()
    main(args.input, args.output, int(args.dim), args.method, args.nodetype)
